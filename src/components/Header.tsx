  ...
  return (
    ...
    >
      <nav className="container mx-auto px-6 md:px-12 lg:px-24 py-4 flex justify-between items-center">
        <a href="#hero" className="text-2xl font-bold text-green font-mono z-50">LT</a>
        
        {/* Desktop Menu */}
        <ul className="hidden md:flex items-center space-x-6">
          {navLinks.map((link, index) => (
            <li key={link.name}>
              <a href={link.href} className="text-light-slate hover:text-green transition-colors duration-300">
                <span className="text-green font-mono">0{index + 1}.</span> {link.name}
              </a>
            </li>
          ))}
          <li>
            <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" className="px-4 py-2 border border-green text-green rounded-md hover:bg-green/10 transition-colors duration-300">
              Resume
            </a>
          </li>
        </ul>
